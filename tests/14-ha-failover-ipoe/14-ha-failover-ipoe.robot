...
*** Comments ***
HA seamless failover test for IPoE with CGNAT.
Establishes sessions on ACTIVE, verifies sync, hard-kills the active BNG,
and validates that sessions and CGNAT mappings are restored on the new active
without subscriber renegotiation. Traffic streams are NOT stopped before failover.
...
*** Variables ***
${lab-name}         osvbng-ha-failover-ipoe
${lab-file}         ${CURDIR}/14-ha-failover-ipoe.clab.yml
