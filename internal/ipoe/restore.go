...
func (c *Component) handleHAStateChange(event events.Event) {
	...
	if isActive && !wasActive && wasStandby {
		c.logger.Debug("SRG promoted from standby, restoring synced IPoE sessions", "srg", data.SRGName)
...
// ipoe-input keys sessions on the sub-interface the packets ingress on, so
// the (parent-interface, svlan) pair from subscriber-group config maps to
// the conventional "parent.svlan" sub-interface name. Falls back to the SRG
// interface for untagged setups or when no subscriber group matches.
